...
async def _place(login, password, signal, lot, tp, sl, orders):
    ...
    try:
        ...
        for _ in range(orders):
            try:
                if signal=="BUY":
                    await conn.create_market_buy_order("XAUUSD", lot, stop_loss=sl, take_profit=tp)
                else:
                    await conn.create_market_sell_order("XAUUSD", lot, stop_loss=sl, take_profit=tp)
                placed += 1
            except Exception as e:
...
def main():
    ...
    while True:
        ...
        # ── FRIDAY TIGHT LOOP ──
        if is_friday() and utc.hour==21 and utc.minute>=55:
            hedge_fired=False
            while True:
                utc=datetime.now(timezone.utc)
                if utc.hour==22: break
                if not hedge_fired and utc.hour==21 and utc.minute==59 and utc.second>=55:
                    p=get_gold_price()
                    if p: friday_hedge(p)
                    hedge_fired=True
                time.sleep(1)
