...
def main():
    ...
    while True:
        ...
        # ── TRACK MODE (threshold 1 — observation only, no trades) ──
        ...
        if not track_state["in_trade"]:
            td2, tt2, tfc2, _, _, _ = analyze_all_timeframes_track(price)
            if td2 in ["BUY", "SELL"] and td2 != track_last_signal[0]:
                ts2 = TRADE_SETTINGS[tt2]
                ...
                track_state["sl"]       = t_sl
                send_telegram(
                    f"[TRACK] {td2} ({ts2['label']}) — observation only\n"
                    f"Entry: ${price:,.2f}\n"
                    f"TP: ${t_tp:,.2f} | SL: ${t_sl:,.2f}\n"
                    f"TF: {tfc2}/3 | Time: {now}\n"
