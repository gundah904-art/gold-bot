...
def get_trend_bos(c, lookback=2):
    """Trend via structural break (BOS) instead of a raw close comparison.
    lookback=2 needs 5 candles — same 20-hour resolution as get_trend on 4h."""
    if len(c) < lookback*2+1: return None
    r = c[-(lookback*2+1):-1]
...
def main():
    ...
    send_telegram(
        ...
        "2. Track Signal — 4h trend + 15m ICT/SMC entry (both accounts)\n"
    ...
    while True:
        ...
        if not track_state["in_trade"]:
            trend_4h=get_trend_bos(candles_4h)
