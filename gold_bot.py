...
def friday_close_hedge(price):
    ...
    if utc_hour == 21 and utc_min == 59 and utc_sec >= 50:
        if len(candles_15m) >= 5:
            ...
            if avg_range > 15:
                ...
                sell_placed = place_trade("SELL", 0.05, 0, sell_sl, orders=1)
                send_telegram(
                    f"FRIDAY HEDGE PLACED! (10s to close)\n"
                    f"Gold: ${price:,.2f}\nVolatility: LOW SAFE!\n"
                    f"-------------------\n"
                    f"BUY 0.05 lot @ ${price:,.2f} | SL: ${buy_sl:,.2f} — {'✅ Placed' if buy_placed else '❌ Failed'}\n"
                    f"SELL 0.05 lot @ ${price:,.2f} | SL: ${sell_sl:,.2f} — {'✅ Placed' if sell_placed else '❌ Failed'}\n"
                    f"-------------------\n"
