...
def check_real_positions(smc_state, zone_state, track_state, track_results, now):
    ...
    try:
        ...
        for deal in deals:
            ...
            if smc_state["in_trade"] and smc_state["entry"]:
                ...
                break
            # Match to Zone
            if zone_state["in_trade"] and zone_state["entry"]:
                send_telegram(
                    f"[ZONE] ✅ Real close from JustMarkets!\n"
                    f"Entry: ${zone_state['entry']:,.2f} → Exit: ${close_px:,.2f}\n"
                    f"Profit: ${profit:,.2f} — {'WIN' if result=='WIN' else 'LOSS'}\nTime: {now}"
                )
                log_trade("ZONE", zone_state["trade_type"], zone_state["entry"], close_px, result)
                zone_state["in_trade"] = False
                break
    except Exception as e:
    ...
    if track_state["in_trade"] and MT_LOGIN2:
        try:
            ...
            for deal in deals2:
                ...
                track_results.append({"direction": track_state["direction"], "result": "TP" if result == "WIN" else "SL"})
...
def main():
    ...
    while True:
        ...
        if smc_state["in_trade"] or zone_state["in_trade"] or track_state["in_trade"]:
            check_real_positions(smc_state, zone_state, track_state, track_results, now)
        ...
        if not track_state["in_trade"]:
            ...
            if trend_4h in ["BULLISH", "BEARISH"] and len(candles_4h) >= 5 and len(candles_15m) >= 3:
                sup_4h, res_4h, dem_4h, sup2_4h = find_zones(candles_4h)
                actual_dir = "BUY" if trend_4h == "BULLISH" else "SELL"
                key_level  = sup_4h if actual_dir == "BUY" else res_4h
                ...
                if key_level is not None:
                    ...
                    # PRE-RETEST: price approaching level (within $5-15), 15m shows momentum
                    pre_retest = (5 <= dist <= 15 and
                                  (actual_dir == "BUY" and curr["close"] > curr["open"] or
                                   actual_dir == "SELL" and curr["close"] < curr["open"]))

                    # POST-RETEST: price touched level, 15m rejection candle confirms bounce
                    post_retest = (dist <= ZONE_TOLERANCE and
                                   (actual_dir == "BUY" and
                                    prev["low"] <= key_level + ZONE_TOLERANCE and
                                    prev["close"] > key_level and
                                    curr["close"] > curr["open"])
                                   or
                                   (actual_dir == "SELL" and
                                    prev["high"] >= key_level - ZONE_TOLERANCE and
                                    prev["close"] < key_level and
                                    curr["close"] < curr["open"]))

                    entry_type = "POST-RETEST" if post_retest else "PRE-RETEST" if pre_retest else None

                    if entry_type and actual_dir != track_last_signal[0]:
                        ...
                        track_state["direction"] = actual_dir
                        ...
                        send_telegram(
                            f"[TRACK] {actual_dir} — {entry_type}\n"
                            f"4h Trend: {trend_4h} | Level: ${key_level:,.2f}\n"
                            f"Entry: ${price:,.2f} | Dist: ${dist:,.2f}\n"
