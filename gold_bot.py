...
def save_state(smc_state, track_state):
    """Persist in-trade state so a restart doesn't forget an open batch of orders."""
    try:
        with open(STATE_FILE, "w") as f:
            json.dump({"smc_state": smc_state, "track_state": track_state}, f)
    except Exception as e:
...
def load_state():
    ...
    try:
        with open(STATE_FILE, "r") as f:
            data = json.load(f)
        return data.get("smc_state"), data.get("track_state")
    except Exception as e:
...
# ─────────────────────────────────────────
# FRIDAY HEDGE — 20:57:40 UTC (23:57:40 EAT)
# ─────────────────────────────────────────
...
def main():
    ...
    send_telegram(
        ...
        "3. Friday Hedge — Auto BUY+SELL at 20:57:40 UTC (23:57:40 EAT)\n"
        f"({ORDERS_PER_SIGNAL} orders/signal) | Account: {MT_LOGIN} ({MT_SERVER})\n"
        "Sessions: Tokyo + London + New York"
    )

    saved_smc, saved_track = load_state()
    smc_state    = saved_smc if saved_smc else new_trade_state()
    last_session = None
    last_weekly  = None
    hedge_fired  = False
    track_state       = saved_track if saved_track else {"in_trade":False,"direction":None,"entry":None,"tp":None,"sl":None}
    ...
    while True:
        ...
        if is_friday() and utc.hour==20 and utc.minute>=52:
            ...
            while True:
                ...
                if not hedge_fired and utc.hour==20 and utc.minute==57 and utc.second>=40:
                    p=get_gold_price()
                    if p: friday_hedge(p)
                    hedge_fired=True
                time.sleep(1)
        ...
        # ── TRAIL STOP-LOSS on open positions (both accounts) ──
        if smc_state["in_trade"]:
            trail_stop(MT_LOGIN, MT_PASSWORD)
        if track_state["in_trade"]:
            trail_stop(MT_LOGIN2, MT_PASSWORD2)
        ...
        if not track_state["in_trade"]:
            ...
            if trend_4h in ["BULLISH","BEARISH"]:
                ...
                if True:
                    t_tp=round(price+6,2) if actual_dir=="BUY" else round(price-6,2)
                    t_sl=round(price-3,2) if actual_dir=="BUY" else round(price+3,2)
        ...
        save_state(smc_state, track_state)
