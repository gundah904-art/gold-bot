...
async def _init_connections():
    global _api, _connection1, _connection2, _symbol
    from metaapi_cloud_sdk import MetaApi
    _api = MetaApi(METAAPI_TOKEN, {'region': 'london'})
    accounts = await _api.metatrader_account_api.get_accounts_with_infinite_scroll_pagination()

    acc1 = next((a for a in accounts if str(a.login) == str(MT_LOGIN)), None)
    if acc1 is None:
        acc1 = await _api.metatrader_account_api.create_account({
            'name': 'Edgar Gold Bot',
            'type': 'cloud',
            'login': MT_LOGIN,
    ...
    if acc1.state not in ['DEPLOYED', 'DEPLOYING']:
        await acc1.deploy()
    await acc1.wait_deployed()
    _connection1 = acc1.get_rpc_connection()
    await _connection1.connect()
    await _connection1.wait_synchronized()
    try:
        symbols = await _connection1.get_symbols()
        for s in symbols:
        ...
        pass
    print(f"Account 1 connected. Symbol: {_symbol}")

    if MT_LOGIN2:
        acc2 = next((a for a in accounts if str(a.login) == str(MT_LOGIN2)), None)
        if acc2 is None:
            acc2 = await _api.metatrader_account_api.create_account({
                'name': 'Edgar Gold Bot Account 2',
                'type': 'cloud',
                'login': MT_LOGIN2,
                'password': MT_PASSWORD2,
                'server': MT_SERVER,
                'platform': 'mt5',
                'magic': 123457
            })
        if acc2.state not in ['DEPLOYED', 'DEPLOYING']:
            await acc2.deploy()
        await acc2.wait_deployed()
        _connection2 = acc2.get_rpc_connection()
        await _connection2.connect()
        await _connection2.wait_synchronized()
        print("Account 2 connected.")

def init_connections():
    try:
        asyncio.run(_init_connections())
        send_telegram("MetaAPI connected to both accounts. Bot ready to trade!")
        return True
    except Exception as e:
        print(f"MetaAPI connection error: {e}")
        send_telegram(f"MetaAPI connection failed: {e}\nBot running in signal-only mode.")
        return False

async def _place_order_async(connection, signal, lot, tp, sl, orders=1):
    placed = 0
    # Ensure connection is synchronized before placing orders
    try:
        await connection.wait_synchronized(timeout_in_seconds=30)
    except Exception as e:
        print(f"Connection sync warning: {e}")
...
def place_trade(signal, lot, tp, sl, orders=ORDERS_PER_SIGNAL):
    if not AUTO_TRADE or _connection1 is None:
        return 0
    try:
        return asyncio.run(_place_order_async(_connection1, signal, lot, tp, sl, orders))
    except Exception as e:
        print(f"Trade wrapper error: {e}")
        return 0

def place_trade_account2(signal, lot, tp, sl):
    if not AUTO_TRADE or _connection2 is None:
        return False
    try:
        placed = asyncio.run(_place_order_async(_connection2, signal, lot, tp, sl, 1))
        return placed > 0
    except Exception as e:
        print(f"Account2 trade wrapper error: {e}")
        return False

# ─────────────────────────────────────────
# FRIDAY CLOSE — AUTO HEDGE (BUY + SELL at market close)
# ─────────────────────────────────────────
def friday_close_hedge(price):
    utc_now  = datetime.now(timezone.utc)
    utc_hour = utc_now.hour
    utc_min  = utc_now.minute
    utc_sec  = utc_now.second
    now      = datetime.now().strftime("%H:%M:%S")
    if utc_hour == 21 and utc_min == 59 and utc_sec >= 55:
        if len(candles_15m) >= 5:
            ranges = [c["high"]-c["low"] for c in candles_15m[-5:]]
            avg_range = sum(ranges)/len(ranges)
            if avg_range > 15:
                send_telegram(
                    f"FRIDAY WARNING!\nToo volatile - SKIP weekend hedge!\n"
                ...
                # Account 1 — BUY
                buy_placed  = place_trade("BUY",  0.05, 0, buy_sl,  orders=1)
                # Account 2 — SELL
                sell_placed = place_trade_account2("SELL", 0.05, 0, sell_sl)
                send_telegram(
                    f"FRIDAY HEDGE PLACED! (10s to close)\n"
                    f"Gold: ${price:,.2f}\nVolatility: LOW SAFE!\n"
                    f"-------------------\n"
                    f"ACCOUNT 1 BUY 0.05 lot @ ${price:,.2f} | SL: ${buy_sl:,.2f} — {'✅ Placed' if buy_placed else '❌ Failed'}\n"
                    f"ACCOUNT 2 SELL 0.05 lot @ ${price:,.2f} | SL: ${sell_sl:,.2f} — {'✅ Placed' if sell_placed else '❌ Failed'}\n"
                    f"-------------------\n"
...
def check_tp_sl(name, state, last_high, last_low, now):
    ...
    if state["trade_type"] == "BUY":
        ...
        if tp_hit and sl_hit:
            # Both hit in same candle — use candle direction to decide
...
def main():
    ...
    # Connect to MetaAPI once at startup
    ...
    while True:
        ...
        if is_friday() and price:
            utc_now = datetime.now(timezone.utc)
            # Enter tight 1-second loop from 21:55 to 22:00 UTC
            if utc_now.hour == 21 and utc_now.minute >= 55:
                print("[FRIDAY] Entering tight loop for market close hedge...")
                hedge_fired = False
                while True:
                    utc_now = datetime.now(timezone.utc)
                    if utc_now.hour == 22:
                        print("[FRIDAY] Market closed. Exiting tight loop.")
                        break
                    if not hedge_fired and utc_now.hour == 21 and utc_now.minute == 59 and utc_now.second >= 55:
                        price = get_gold_price()
                        if price:
                            friday_close_hedge(price)
                        hedge_fired = True
                    time.sleep(1)
        ...
        # ── KEEP METAAPI CONNECTION ALIVE ──
        if _connection1 is not None:
            try:
                asyncio.run(_connection1.wait_synchronized(timeout_in_seconds=5))
            except:
                pass
