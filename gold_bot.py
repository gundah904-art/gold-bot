...
async def _place_trade_async(signal, lot, tp, sl, orders=ORDERS_PER_SIGNAL):
    ...
    try:
        ...
        # Get exact symbol name from broker
        ...
        for _ in range(orders):
            try:
                if signal == "BUY":
                    await connection.create_market_buy_order(symbol, lot, stop_loss=sl, take_profit=tp)
                else:
                    await connection.create_market_sell_order(symbol, lot, stop_loss=sl, take_profit=tp)
                placed += 1
...
# ─────────────────────────────────────────
# METAAPI — PLACE TRADE ON ACCOUNT 2 (Friday SELL hedge)
# ─────────────────────────────────────────
async def _place_trade_account2_async(signal, lot, tp, sl):
    try:
        from metaapi_cloud_sdk import MetaApi
        api = MetaApi(METAAPI_TOKEN, {'region': 'london'})
        accounts = await api.metatrader_account_api.get_accounts_with_infinite_scroll_pagination()
        account = None
        for acc in accounts:
            if str(acc.login) == str(MT_LOGIN2):
                account = acc
                break
        if account is None:
            account = await api.metatrader_account_api.create_account({
        ...
        print(f"MetaAPI Account2 trade error: {e}")
        return False
...
def place_trade_account2(signal, lot, tp, sl):
    if not AUTO_TRADE:
        return False
    try:
        return asyncio.run(_place_trade_account2_async(signal, lot, tp, sl))
    except Exception as e:
