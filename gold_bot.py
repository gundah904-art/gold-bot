...
def analyze_zone_signal(price):
    ...
    prev = candles_15m[-2]  # previous completed candle
    curr = candles_15m[-1]  # current candle

    # Demand zone — look for rejection (wick down into zone, body closes above)
    if demand is not None and abs(price - demand) <= ZONE_TOLERANCE:
        # Rejection candle: previous candle wicked INTO zone but closed ABOVE it
        if prev["low"] <= demand + ZONE_TOLERANCE and prev["close"] > demand:
            return "BUY", demand, support, resistance, demand, supply

    # Supply zone — look for rejection (wick up into zone, body closes below)
    if supply is not None and abs(price - supply) <= ZONE_TOLERANCE:
        # Rejection candle: previous candle wicked INTO zone but closed BELOW it
...
def send_weekly_summary(track_results=[]):
    ...
    if track_results:
        ...
        track_text=(
            ...
            f"Total: {t_total} | TP: {t_tp} | SL: {t_sl}\n"
            f"Win Rate: {t_wr}%\n{dir_text}"
            f"{'⚠️ Consider reversing signals!' if t_wr < 40 else '✅ Signals look valid' if t_wr > 60 else '⚖️ Mixed - need more data'}"
        )
...
def main():
    ...
    track_results = []  # stores {"direction": ..., "result": "TP"/"SL"}
    ...
    while True:
        ...
        # ── TRACK SIGNAL (threshold 1 — no trades, auto monitors TP/SL) ──
        # Check if current track signal hit TP or SL
        if track_state["in_trade"]:
            if track_state["direction"] == "BUY":
                if lh >= track_state["tp"]:
                    send_telegram(f"[TRACK] BUY hit TP ✅\nEntry: ${track_state['entry']:,.2f} → TP: ${track_state['tp']:,.2f}\nResult: Would have WON")
                    track_results.append({"direction": "BUY", "result": "TP"})
                    track_state["in_trade"] = False
                elif ll <= track_state["sl"]:
                    send_telegram(f"[TRACK] BUY hit SL ❌\nEntry: ${track_state['entry']:,.2f} → SL: ${track_state['sl']:,.2f}\nResult: Would have LOST")
                    track_results.append({"direction": "BUY", "result": "SL"})
                    track_state["in_trade"] = False
            elif track_state["direction"] == "SELL":
        ...
        # Fire new track signal if not already tracking
        if not track_state["in_trade"]:
            ...
            if track_d in ["BUY","SELL"] and track_d!=track_last_signal[0]:
                ...
                track_state["direction"]=track_d
                track_state["entry"]=price
                track_state["tp"]=track_tp
                track_state["sl"]=track_sl
                send_telegram(
                    f"[TRACK] {track_d} ({track_s['label']}) — observation only\n"
                    f"Entry: ${price:,.2f}\n"
