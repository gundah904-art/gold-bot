...
async def _place_trade_async(signal, lot, tp, sl, orders=ORDERS_PER_SIGNAL, login=None, password=None):
    if login is None:
        login = MT_LOGIN
    if password is None:
...
def place_trade_account2(signal, lot, tp, sl):
    if not AUTO_TRADE:
        return False
    try:
        placed = asyncio.run(_place_trade_async(signal, lot, tp, sl, 1, MT_LOGIN2, MT_PASSWORD2))
        return placed > 0
    except Exception as e:
...
def init_connections():
    send_telegram("Bot started. MetaAPI will connect on first signal.")
    return True
